...
"""Reserved-name placeholder for ``idp-accelerator-cli`` — NOT the real package.

This distribution exists only to hold the name ``idp-accelerator-cli`` on public
...
The real ``idp_cli`` is first-party: it lives in the accelerator repo at
``lib/idp_cli_pkg`` and is installed from a local checkout, never from PyPI.
...
_MESSAGE = """\
idp-accelerator-cli from PyPI is a NAME PLACEHOLDER, not a functional package.

You have installed a reserved-name stub published by AWS to prevent dependency
confusion. It contains no code.

The real idp_cli is part of the GenAI IDP Accelerator and is installed
