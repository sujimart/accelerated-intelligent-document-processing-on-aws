...
"""Reserved-name placeholder for ``idp-mcp-connector`` — NOT the real package.

This distribution exists only to hold the name ``idp-mcp-connector`` on public
...
The real ``idp_mcp_connector`` is first-party: it lives in the accelerator repo at
``lib/idp_mcp_connector_pkg`` and is installed from a local checkout, never from PyPI.
...
_MESSAGE = """\
idp-mcp-connector from PyPI is a NAME PLACEHOLDER, not a functional package.

You have installed a reserved-name stub published by AWS to prevent dependency
confusion. It contains no code.

The real idp_mcp_connector is part of the GenAI IDP Accelerator and is installed
